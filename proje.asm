...
.data
    ...
    buffer db 100, 0   ; Metin giriþi için kullanýlan tampon bellek
...
.code   
    ...
    klavye_metin_girisi:
        mov ah, 0       ; AH kaydedicisini sýfýrla
        int 16h         ; Klavyeden bir tuþa basýlmasýný bekler
    
        cmp ah, 0       ; AH kaydedicisini 0 ile karþýlaþtýrarak tuþun basýlý olup olmadýðýný kontrol et
        je klavye_metin_girisi   ; Tuþ basýlý deðilse tekrar tuþa basýlmasýný bekler
    
        mov dl, al      ; Giriþ tuþunu DL kaydedicisine aktar
        cmp dl, 0DH     ; Giriþ tuþu Enter tuþu (0DH) ise döngüden çýk
        je klavye_metin_girisi_cikis
    
        ; Giriþ tuþunu kullanarak yapmak istediðiniz iþlemi gerçekleþtirin
        ; Örneðin, tuþun ASCII deðerini yazdýrabilirsiniz
        mov ah, 2       ; DOS iþlevi çaðýrmak için kullanýlan AH kaydedicisi
        int 21h         ; AH=2 olduðunda karakteri yazdýran DOS kesmesi
    
        ; Giriþ tuþunu buffer'ýn sonuna ekleyin
        mov si, 0       ; SI kaydedicisini sýfýrla
    klavye_metin_girisi_ekle:
        cmp byte ptr [buffer+si], 0  ; Buffer'ýn sonunu kontrol et
        jne klavye_metin_girisi_sonraki  ; Buffer'ýn sonu deðilse sonraki byte'a geç
        mov byte ptr [buffer+si], dl   ; Giriþ tuþunu buffer'a ekle
        inc si                         ; SI kaydedicisini bir sonraki byte'a hareket ettir
        jmp klavye_metin_girisi        ; Bir sonraki tuþa basýlmasýný beklemek için döngüye geri dön
    
    klavye_metin_girisi_sonraki:
        inc si                         ; SI kaydedicisini bir sonraki byte'a hareket ettir
        jmp klavye_metin_girisi_ekle   ; Giriþ tuþunu buffer'a ekle
    ...
    Secim_Sonuc_Karsilastirma: 
        mov bx, 0    
        mov bh, [buffer+3]   ; 1. seçimin sonucu BH a atanýyor
        mov bl, [buffer+1]   ; 2. seçimin sonucu BL ye atanýyor 
 
        ; 1. Seçim için karþýlaþtýrmalar       
        cmp bh, '1'
        je sonuc4
    ...
    dogruExit:
        ...
        mov ah, 4Ch     ; AH=4Ch - Programý sonlandýrmak için kesmeyi çaðýr
        int 21h
    yanlisExit: 
        mov ax, @data
        mov ds, ax
